...
const Editor = () => {
    ...
    const { themeData, checkProfile, setCheckProfile, checkLinkedin, setCheckLinkedin, checkGithub, setCheckGithub, checkAward, setCheckAward, setThemeData, checkProj, checkWork, setCheckProj, setCheckWork } = useContext(ResumeContext)
    ...
    const [personalData, setPersonalData] = useState({
        profileImage: 'https://www.w3schools.com/howto/img_avatar.png', name: "Your Name", summary: 'Lorem ipsum dolor sit amet, consectetur adipiscing eli', profile: "Work Profile", address: "Address Line", phone: "Phone Number", email: "Email Address", linkedinUrl: "Your linkedin profile url",
        githubUrl: "Your github profile url", skill: 'Your, Skills, are, shown, here',
    })
    const [awardData, setAwardData] = useState({ awards: 'Achievement 1, Achievement 2' })
    ...
    const handleChangePersonal = (e) => {
        ...
        if (e.target.name === 'linkedinUrl') {
            setPersonalData({ ...personalData, linkedinUrl: e.target.value })
        }
        if (e.target.name === 'githubUrl') {
            setPersonalData({ ...personalData, githubUrl: e.target.value })
        }
    }
    ...
    const handleWorkClick = (e) => {
        ...
        const template = (
            <>
                ...
                <FormControl isRequired className='my-2'>
                    <Textarea id={`wDescription${i}`} name='wDescription' onChange={handleChangeWork} placeholder='Use comma to separate Description e.g. project summary, technologies used' />
                </FormControl>
            </>
        )
        ...
        <>
            <div id="form-collect">
                ...
                <div id="form-personal" className='mb-2'>
                    ...
                    <FormControl isRequired className='my-2'>
                        <div className='file'>
                            ...
                            <input accept="image/*" name='profileImage' onChange={handleChangePersonal} id='input-file' type='file' disabled={checkProfile} />
                            <img className="blah" src={personalData.profileImage} alt="your profile preview" />
                        </div>
                        <Switch defaultChecked={true} onChange={() => (setCheckProfile(!checkProfile))} colorScheme='teal' />
                    </FormControl>
                    <FormControl isRequired className='my-2'>
                    ...
                    <FormControl className='my-2'>
                        <Input name='linkedinUrl' onChange={handleChangePersonal} type={'text'} placeholder='Linkedin url' disabled={checkLinkedin} />
                        <Switch defaultChecked={true} onChange={() => (setCheckLinkedin(!checkLinkedin))} colorScheme='teal' />
                    </FormControl>
                    <FormControl className='my-2'>
                        <Input name='githubUrl' onChange={handleChangePersonal} type={'text'} placeholder='Github url' disabled={checkGithub} />
                ...
                <div id="form-personal" className='mb-2'>
                    ...
                    <hr />
                    <FormControl isRequired className='my-2'>
                        <Textarea name='awards' disabled={checkAward} onChange={handleChangeAwards} placeholder='Use comma to separate Achievement e.g. achievement 1, achievement 2' />
                    </FormControl>
                </div>
            </div>
        </>
